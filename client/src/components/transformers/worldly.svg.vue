...
export default {
	...
	computed: {
		...
		viewPoint() {
			return (
				this.translateAdjust(this.world.getViewPoint.x) + ' ' + this.translateAdjust(this.world.getViewPoint.y)
			)
		},
		lockedPercentage() {
		...
		scale() {
			return (
				this.lockedPercentage + (this.world.getZoomLevel / this.restrainedDepth) * (1 - this.lockedPercentage)
			)
		},
	},
}
