...
export default {
	data() {
		return {
			...
			words: [
				['By the year 2140,', 'every planet in our solar', 'system is overpopulated.'],
				['Still bound to our sun,', "we've continued our longing", 'for the stars.'],
				['That was until today!', 'The first starships', 'have arrived.'],
				['Orbiter 8 is an MMO', 'galaxy-exploring strategy', 'game for the EVM.'],
				['Introducing Portals!', 'Starships will soon cross', 'more than just the galaxy.'],
				['Portals are Bridges', 'connecting distinct universes', 'into a multiverse.'],
			],
			backgroundScale: [2, 1.5, 4, 2, 1.5, 0.75],
			waitTime: [3000, 7000, 14000, 9000, 8000, 10000],
			lastScale: 1,
		}
	},
	mounted() {
...
<template>
	<g>
		...
		<panelNavigation :panel-count="panelCount" @panel="p => (panel = p)" @finish="finish()" @close="close()" />
	</g>
</template>
