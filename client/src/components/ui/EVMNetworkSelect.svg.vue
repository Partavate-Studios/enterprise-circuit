	...
	<g font-size="26px">
		...
		<g transform="scale(1)">
			<g transform="translate(-270 -300)">
				<g
					v-for="(network, key, index) in deployments"
					:transform="'translate(0 ' + index * 60 + ')'"
					@mouseenter="this.hoverId = key"
					@mouseleave="this.hoverId = null"
				>
					<btn
						:val="key"
						:width="evm.chainId == key ? 230 : 210"
						:height="48"
						@click="switchNetwork"
						font-size="0.9em"
			...
			<g transform="translate(110 -330)" v-if="networkToShow != ''">
				...
				<g v-if="networks[networkToShow]">
					<text font-size="26px" transform="translate(0 40)">{{
						networks[networkToShow] ? networks[networkToShow].name : 'Select A Network'
					}}</text>
					<g font-size="20px">
						<text transform="translate(0 80)"
							>Native Token: {{ networks[networkToShow].currency.symbol }}</text
						>
				...
				<g v-if="evm.chainId == networkToShow">
					<g transform="translate(0 560)" v-if="evm.isSuppportedNetwork">
						<btn
							...
							@click="$emit('close')"
							font-size="32px"
							:text="'Play on ' + networks[networkToShow].shortName"
						/>
					</g>
					...
					<g transform="translate(0 200)" font-size="24px">
						...
						<text
							...
							class="canclick"
							@click="openNewWindow(evm.explorer + '/address/' + world.avatar.avatarContractAddress)"
							fill="#ffffaa"
							font-size="16px"
							font-family="monospace"
							>{{ world.avatar.avatarContractAddress }}</text
						>

						<text transform="translate(-10 60)" text-anchor="end" fill="#aaaaaa">Galaxy Contract:</text>
						<text
							v-if="world.galaxy.isConnected"
						...
						<text
							...
							class="canclick"
							@click="openNewWindow(evm.explorer + '/address/' + world.galaxy.galaxyContractAddress)"
							fill="#ffffaa"
							font-size="16px"
							font-family="monospace"
							>{{ world.galaxy.galaxyContractAddress }}</text
						>
					</g>
					...
					<g font-size="20px" transform="translate(0 360)">
						<g transform="translate(0 0)">
							...
							<text transform="translate(10 0)" text-anchor="start"
								>{{ evm.balance }} {{ evm.currencyData.symbol }}</text
							>
						</g>

						<g transform="translate(0 50)">
							<text transform="translate(-50 -5)" text-anchor="end" fill="#aaaaaa"
								>Get more {{ evm.currencyData.symbol }}:</text
							>
							<g v-if="evm.faucets.length > 0">
								<g v-for="(faucet, index) in evm.faucets">
									<btn
				...
				<g v-else>
					<g transform="translate(0 320)">
						<btn
							...
							:val="networkToShow"
							font-size="28px"
							:text="'Change to ' + networks[networkToShow].shortName"
						/>
					</g>
					<g transform="translate(0 620)">
						<btn
							:width="260"
							:height="50"
							@click="networkSelected = evm.chainId"
							:val="networkToShow"
							font-size="22px"
							text="Cancel"
						/>
		...
		<g v-if="evm.switchingNetwork">
			<rect :x="-350" :y="-350" :width="700" :height="700" fill-opacity="0.75" fill="#202426" rx="10" ry="10" />
			<spinner />
		</g>
	</g>
</template>
