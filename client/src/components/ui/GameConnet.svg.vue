...
<template>
	...
	<g v-if="evm.isConnected">
		<g font-size="28px" transform="translate(0 -140)">
			...
			<text transform="translate(-40 0)" text-anchor="start"
		...
		<g font-size="28px" transform="translate(0 -40)">
			...
			<text v-if="avatar.isConnected" transform="translate(-40 0)" text-anchor="start" fill="#88ff88"
				>Connected</text
			>
			...
			<text
				...
				class="canclick"
				@click="openNewWindow(evm.explorer + '/address/' + avatar.avatarContractAddress)"
				fill="#ffffaa"
				font-size="24px"
				font-family="monospace"
				>{{ avatar.avatarContractAddress }}</text
			>

			<text transform="translate(-50 70)" text-anchor="end" fill="#aaaaaa">Galaxy Contract:</text>
			<text v-if="galaxy.isConnected" transform="translate(-40 70)" text-anchor="start" fill="#88ff88"
				>Connected</text
			>
			...
			<text
				...
				class="canclick"
				@click="openNewWindow(evm.explorer + '/address/' + galaxy.galaxyContractAddress)"
				fill="#ffffaa"
				font-size="24px"
				font-family="monospace"
				>{{ galaxy.galaxyContractAddress }}</text
			>
		</g>
		...
		<g v-if="avatar.isLoaded && galaxy.isLoaded" transform="translate(0 120)">
			<g v-if="world.avatar.playerHasAvatar">
				<g font-size="28px" transform="translate(0 0)">
					<text transform="translate(-5 0)" fill="#aaaaaa"
						>Welcome back {{ avatar.chainstate.myAvatarName }}.</text
					>
			...
			<g font-size="28px" fill="#aaaaaa" transform="translate(0 40)">
				<text
					...
					{{ galaxy.chainstate.systemCount }} stars.</text
				>
			</g>
			...
			<g v-if="galaxy.chainstate.shipId > 0">
				<g font-size="28px" transform="translate(0 80)">
					<text transform="translate(-5 0)" fill="#aaaaaa"
						...
						{{ galaxy.chainstate.systemData.name }}</text
					>
				</g>
			</g>
		</g>
	...
	<g :transform="'translate(0 ' + (screen.bottom - 200) + ')'">
		<g v-if="!evm.isConnected">
			...
			<btn fill="#ffffff" :width="500" :height="90" text="Connect" @click="evm.connect()" />
		</g>
		<g v-else-if="!evm.isSuppportedNetwork">
			<text font-size="40px" :transform="'translate(0 ' + -100 + ')'">Please select a supported network.</text>
			<btn fill="#ffffff" :width="500" :height="90" text="Change" @click="screen.evm.showNetworkSelect = true" />
		</g>
		...
		<g v-else>
			<btn fill="#ffffff" :width="500" :height="90" font-size="50px" text="Play" @click="$emit('readyToPlay')" />
		</g>
	</g>
