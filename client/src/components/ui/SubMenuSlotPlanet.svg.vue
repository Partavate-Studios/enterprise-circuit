	...
	computed: {
		...
		moons() {
			return this.world.sprites.filter(sprite => {
				return sprite.type == 'Moon' && sprite.orbit.parent == this.planetIndex
			})
		},
		station() {
			return this.world.sprites.filter(sprite => {
				return sprite.type == 'Station' && sprite.orbit.parent == this.planetIndex
			})
		},
	},
}
	...
	<g>
		...
		<g transform="translate(-50 -20)" v-if="station.length > 0">
			<g transform="translate(0 0)">
				...
				<g transform="">
					<text text-anchor="start" font-size="22px" fill="#aaaaaa"
						>[{{ station[0].getPortType() }}] {{ station[0].meta.name }}</text
					>
				</g>
			</g>
		</g>
