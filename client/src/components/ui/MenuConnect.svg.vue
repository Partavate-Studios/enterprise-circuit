...
<template>
	...
	<g v-if="!evm.isConnected">
		<g :transform="'translate(0 ' + (screen.top + 120) + ')'">
			<connectionIndicator :walletConnected="evm.hasWallet" :networkConnected="false" />
		</g>
	...
	<g v-else>
		<g :transform="'translate(0 ' + (screen.top + 30) + ')'">
			...
			<g v-if="world.isLoaded" transform="translate(0 0)" font-size="24px" fill="#888888">
				<g transform="translate(-250 80)" text-anchor="start">
					<text
						...
						{{ world.galaxy.chainstate.systemCount }} star systems.</text
					>
				</g>
				...
				<g v-if="world.galaxy.haveShip" text-anchor="start">
					<g transform="translate(-250 140)">
						<text
							...
							{{ world.galaxy.chainstate.systemData.name }}</text
						>
					</g>
				</g>
			</g>
			...
			<g font-size="26px" fill="#aaaaaa" transform="translate(0 180)">
				<text transform="translate(-10 0)" text-anchor="end"> Status: </text>
				<g text-anchor="start" transform="translate(-0 0)">
					<g v-if="evm.isSuppportedNetwork">
						<text v-if="world.isLoaded" fill="#aaffaa">Connected</text>
						<text v-else fill="#ffffaa">Loading</text>
					</g>
					<g v-else="evm.isSuppportedNetwork">
						<text fill="#ffaaaa">Unsupported</text>
					</g>
				</g>
			</g>
		</g>
	...
	<g font-size="36px" :transform="'translate(0 ' + (screen.bottom - 20) + ')'">
		<g :transform="'scale(' + ui.UIScale + ')'">
			<btn :width="200" :height="60" text="Close" @click="ui.goHome()" transform="translate(0 -80)" />
		</g>
	</g>
</template>
