...
<template>
	<g :transform="'translate(' + sprite.position.x + ' ' + sprite.position.y + ')'">
		<circle
			...
			v-if="selected"
		/>
		<Star v-if="sprite.type == 'Star'" :size="sprite.meta.size" />
		<Planet
		...
		/>
		<Moon v-if="sprite.type == 'Moon'" :size="sprite.meta.size" />
		<SpaceStation v-if="sprite.type == 'Station'" :size="sprite.meta.size" />
		<Ship v-if="sprite.type == 'Ship'" :size="1" :show-engines="true" :rotation="sprite.rotation" />
	</g>
</template>
